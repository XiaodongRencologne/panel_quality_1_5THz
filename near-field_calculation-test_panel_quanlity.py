...
ad_m2=np.zeros(5*69);
ad_m1=np.zeros((5,77));
m2_panel='M2_error_profile.txt'
m1_panel='M1_error_profile.txt'
m2_panel=None
m1_panel=None
...
#np.savetxt('output/panel_quality/manufacturing_error.txt',np.append(S.real,S.imag).reshape(2,-1).T,delimiter=',');
np.savetxt('output/panel_quality/no-manufacturing_error.txt',np.append(S.real,S.imag).reshape(2,-1).T,delimiter=',');
# In[ ]:
