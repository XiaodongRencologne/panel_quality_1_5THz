...
plt.colorbar();
plt.savefig('output/near_field/near_field_beam.png')
...
# saveing data;
np.savetxt('output/near_field/source_field.txt',np.append(S.real,S.imag).reshape(2,-1).T,delimiter=',');
np.savetxt('output/near_field/imaginary_field.txt',np.append(Field_fimag.real,Field_fimag.imag).reshape(2,-1).T,delimiter=',');
np.savetxt('output/near_field/m1_field.txt',np.append(Field_m1.real,Field_m1.imag).reshape(2,-1).T,delimiter=',');
np.savetxt('output/near_field/m2_field.txt',np.append(Field_m2.real,Field_m2.imag).reshape(2,-1).T,delimiter=',');
